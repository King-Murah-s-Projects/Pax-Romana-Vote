...
export class VoteCountingService {
  ...
  async countVotesForPosition(
    ...
    const voteCounts = this.countVotesPerCandidate(
      candidates,
      positionVotes,
      totalVotes,
    );
    ...
    const unopposedThresholdMet = isUnopposed
      ? voteCounts[0]?.voteCount >=
        totalEligibleVoters * this.UNOPPOSED_THRESHOLD
      : true;
    ...
    const result: PositionResult = {
      ...
      status: this.determineResultStatus(
        voteCounts,
        isUnopposed,
  ...
  ): Record<Candidate_Position, string> {
  ...
  private checkRunoffRequired(voteCounts: VoteCount[]): boolean {
    ...
    return (
      voteCounts[0].voteCount === voteCounts[1].voteCount &&
      voteCounts[0].voteCount > 0
    );
  }
